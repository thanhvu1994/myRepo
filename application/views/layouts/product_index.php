                    ...
                    <ul class="htmlcontent-home clearfix">
                        <?php foreach($categories as $key => $item): ?>
                            <li class="htmlcontent-item-<?php echo $key + 1; ?> col-xs-4" style="text-align: center">
                                <?php if ($this->session->userdata['languages'] == 'vn'): ?>
                                    <a href="<?php echo base_url('danh-muc/'.$item->slug); ?>" class="item-link" onclick="return !window.open(this.href);" title="<?php echo $item->title; ?>">
                                <?php else: ?>
                                    <a href="<?php echo base_url('danh-muc/'.$item->slug_en); ?>" class="item-link" onclick="return !window.open(this.href);" title="<?php echo $item->title; ?>">
                                <?php endif ?>
